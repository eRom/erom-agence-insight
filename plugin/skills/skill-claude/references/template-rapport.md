...
L'ordre est imposé. Un rapport `stop-secu` ne porte que les sections 1 et 2.
...
**5. La véracité.** Le tableau des promesses numérotées : promesse, verdict (`tient`, `ne tient pas`, `invérifiable`), preuve en `fichier:ligne`, et pour `ne tient pas` ce qui est réellement vrai. Puis « ce qu'elle nomme et qui n'existe pas », liste éventuellement vide, dite vide. Puis le fait d'API vérifié contre la documentation officielle, avec l'URL lue et ce qu'elle dit. Puis le jugement de la skill comme skill : déclenchement, taille, disclosure progressive, renvois morts.
...
**8. Couverture et limites.** Obligatoire, jamais omise :
...
- Une ligne citée qui contient un tiret cadratin se recopie avec un tiret simple, suivie de « (cadratin remplacé) » : le hook refuse l'écriture sinon, et la citation reste fidèle au mot près.
